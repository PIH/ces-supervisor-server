...
def show_running():
    ...
    if running_sites:
        print(
            "Ya corriendo:\n"
            + "\n".join(["{}\t({})".format(s, port_for_site(s)) for s in running_sites])
            + "\n"
        )


def run_emr():
    sites_with_running_info = [s + (" (activo)" if is_up(s) else "") for s in SITES]
    site = (
        _get_selection(
            "Elegir un EMR de correr, o (0) para regresar.", sites_with_running_info
        )
        .split(" ")[0]
        .strip()
    )
...
def stop_server():
    ...
    process = sp.Popen(
        "kill $(ps -ef | grep " + site + " | grep -v grep | awk '{print $2}')",
        shell=True,
    )
    process.wait()
    sleep(3)
...
def import_data():
    ...
    site = _get_selection(
        "A cual EMR quieres importar el archivo {}?".format(file_path), SITES
    )
    tmp_dir = mkdtemp()
    print("\n\n=== DECOMPRESSANDO ===\n")
    unzip_process = sp.Popen(["7za", "e", file_path, "-p" + PASSWORD, "-o" + tmp_dir])
    ...
    fix_process = sp.Popen(
        "sed -i -E 's/DEFINER=[^]+@[^]+/DEFINER=`openmrs`/g' " + file_path, shell=True
    )
...
def update_emr():
    base_dir = "/home/sup/pih-emr"
    sp.run("cd " + base_dir + "/openmrs-config-ces; git pull", shell=True)
    sp.run("cd " + base_dir + "/openmrs-module-mirebalais; git pull", shell=True)
    for server in SITES:
        sp.run(
            "cd " + base_dir + "/openmrs-module-mirebalais; "
    ...
    print(
        ...
        "La primera vez que abres cada uno, va a tomar mas tiempo abriendo."
    )
...
def export_users():
    ...
    insert_users = _run_in_docker(
        "mysqldump -uopenmrs -p"
        + PASSWORD
        + " --databases "
        + site
...
def import_users():
    ...
    site = _get_selection(
        "A cual EMR quieres importar usuarios? (0 para regresar)", site_options
    )
    if site == None:
        return
    elif site == "Todos":
...
def launch_browser(site):
    command = (
        "firefox -new-tab http://localhost:" + str(port_for_site(site)) + "/openmrs"
    )
...
def _get_selection(prompt, options):
    ...
    selection_prompt = (
        prompt
        + "\n"
        + "\n".join(["{}) {}".format(i + 1, val) for i, val in enumerate(options)])
        + "\n"
    )
    result = None
